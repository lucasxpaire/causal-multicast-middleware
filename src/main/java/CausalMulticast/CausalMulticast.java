...
/**
 * Componente core do Middleware respons�vel por gerenciar a entrega causal de mensagens.
 * Utiliza o algoritmo de Matriz de Rel�gios L�gicos (Matrix Clock) para rastrear o progresso 
 * de conhecimento global de cada n� na rede distribu�da e aplicar descarte por estabiliza��o.
 */
public class CausalMulticast {

    /** Identificador �nico do n� local, formatado como "IP:Porta". */
    private final String localId;
    
    /** Interface de callback para entregar as mensagens ordenadas para a aplica��o cliente. */
    ...
    /** Mapa que correlaciona o ID textual de um peer (IP:Porta) com seu �ndice num�rico na Matriz de Rel�gios. */
    private Map<String, Integer> peerToIndex;
    
    /** Matriz de Rel�gios Vetoriais de tamanho N x N. Onde matrixClock[i][j] representa o conhecimento que o n� i possui sobre os eventos gerados pelo n� j. */
    private int[][] matrixClock;
    
    /** Buffer tempor�rio e thread-safe para armazenar mensagens recebidas da rede pendentes de valida��o causal ou estabiliza��o. */
    ...
    /** Utilit�rio de rede encarregado de realizar o envio f�sico de pacotes datagrama UDP. */
    private UDPSender udpSender;

    // Fila thread-safe de mensagens retidas na saída do emissor
    private final List<DelayedPacket> outgoingDelayedQueue = Collections.synchronizedList(new ArrayList<>());

    /**
     * Inicializa o middleware de multicast causal para o n� local.
     *  @param ip IP unicast local.
     * @param port Porta unicast local.
     * @param client Refer�ncia de retorno da aplica��o cliente.
     */
    ...
    /**
     * Atualiza os membros do grupo e redimensiona a matriz de rel�gios preservando o hist�rico.
    ...
    public BufferedMessage mcsend(String msg) {
        ...
        synchronized (this) {
            // 1. INCREMENTA ANTES: O n� avan�a seu rel�gio local significando um evento de envio
            this.matrixClock[localPeerIndex][localPeerIndex] = this.matrixClock[localPeerIndex][localPeerIndex] + 1;

            // 2. COPIA O REL�GIO ATUALIZADO: A mensagem vai carregar o carimbo novo
    ...
    /**
     * Varre a lista de n�s ativos no sistema e realiza disparos individuais (unicast) 
     * via UDP com o envelope da mensagem serializada, simulando um canal multicast.
     *  @param message O envelope estruturado contendo dados l�gicos e o texto.
     */
    ...
    /**
     * Realiza o envio físico unicast imediato para um peer específico.
     */
    ...
    /**
     * Callback invocado de forma ass�ncrona pelo receptor UDP assim que pacotes brutos chegam da rede.
     *  @param message A inst�ncia da mensagem encapsulada com os metadados.
     */
    public void onMessageReceived(BufferedMessage message) {
        ...
        synchronized (this) {
            ...
            // 2. Atualiza a linha da matriz do remetente com o rel�gio que veio na mensagem
            for (String peer : this.activePeers) {
            ...
            // 4. Verifica se a mensagem pode ser entregue � aplica��o
            boolean newDelivery;
            do {
                newDelivery = false;
                            ...
                            for (String peer : this.activePeers) {
                                ...
                                if (peerIndex != null) {
                                    ...
                                    if (peer.equals(msgSender)) {
                                        // REGRA 1: Para o remetente, tem que ser exatamente a pr�xima (local + 1)
                                        if (messageClockValue != localClockValue + 1) {
                                            canDeliver = false;
                                            break;
                                        }
                                    } else {
                                        // REGRA 2: Para os outros, o local tem que estar igual ou mais avan�ado que a mensagem
            ...
            // 5. Estabiliza��o e Descarte do Buffer
            ...
            for (BufferedMessage bufferedMsg : this.messageBuffer) {
                if (bufferedMsg.isDelivered()) {
                    ...
                    if (msgSenderIndex != null) {
                        ...
                        // Encontra o menor rel�gio registrado para o remetente entre todos os n�s
                        ...
                        if (msgSeq <= minReceived) {
                            ...
                            System.out.println("[ESTABILIZA��O] Mensagem de " + msgSender + " (\"" + bufferedMsg.getContent() + "\") foi recebida por todos os n�s e foi descartada do buffer.");
                        }
                    }
                }
            }
    ...
    /**
     * Retorna a matriz de rel�gios l�gicos formatada visualmente em texto.
     *  @return String contendo a tabela de estados da matriz de rel�gios locais.
     */
    ...
    public synchronized String getBufferState() {
        ...
        for (BufferedMessage msg : bufferCopy) {
            sb.append("- De: ").append(msg.getSenderId())
                    ...
                    .append(" | Rel�gio da Mensagem: ").append(msg.getVectorClock())
                    .append(" | Entregue � Tela: ").append(msg.isDelivered())
                    .append("\n");
        }
        sb.append("=========================================================");
        return sb.toString();
